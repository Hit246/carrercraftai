...
function AppLayoutContent({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (userData && userData.hasCompletedOnboarding === false) setShowTour(true);
  }, [userData]);
  ...
  const getPlanBadge = () => {
    ...
    if (plan === 'pending') return <Badge variant="secondary" className="ml-auto bg-yellow-400/20 text-yellow-500">Pending</Badge>
    return <Button variant="ghost" size="sm" className="ml-auto" asChild><Link href="/pricing"><Crown className="w-4 h-4 mr-2"/>Upgrade</Link></Button>
  }
  ...
  return (
    ...
    <SidebarProvider>
      <Sidebar>
        <SidebarHeader className="p-4">
          <Link href="/" className="flex items-center gap-2">
            <Image src="/logo.jpg" alt="Logo" width={28} height={28} className="rounded-full object-cover" />
        ...
        <SidebarContent>
          <SidebarMenu>
             ...
             <SidebarMenuItem><SidebarMenuButton asChild isActive={isActive('/cover-letter-generator')}><Link href="/cover-letter-generator"><FileText /><span>Cover Letter</span>{plan === 'free' && <Badge variant="secondary" className="ml-auto">Pro</Badge>}</Link></SidebarMenuButton></SidebarMenuItem>
        ...
        <SidebarFooter className="p-4 mt-auto border-t">
          <div className="flex items-center gap-3">
             <Avatar className="h-9 w-9"><AvatarImage src={userData?.photoURL || `https://placehold.co/100x100.png?text=${user.email?.[0].toUpperCase()}`} /><AvatarFallback>{user.email?.[0].toUpperCase()}</AvatarFallback></Avatar>
            <div className="flex flex-col truncate">
                <span className="text-sm font-medium truncate">{user.displayName || user.email}</span>
                <button onClick={() => logout()} className="text-xs text-muted-foreground hover:text-foreground flex items-center gap-1"><LogOut className="w-3 h-3"/>Logout</button>
            </div>
            {getPlanBadge()}
          </div>
        </SidebarFooter>
      ...
      <SidebarInset>
        <header className="flex h-14 items-center gap-4 border-b bg-card px-4 sm:px-6 shadow-sm">
          <SidebarTrigger /><div className="flex-1" /><ThemeSwitcher /><Button variant="outline" size="icon" asChild><Link href="/profile"><Settings className="h-5 w-5" /></Link></Button>
        </header>
