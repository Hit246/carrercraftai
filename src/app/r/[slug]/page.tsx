...
async function getResumeBySlug(slug: string) {
  try {
    ...
    const snapshot = await db
      .collectionGroup('resumeVersions')
    ...
    <div className="min-h-screen bg-slate-50 py-12 px-4 md:px-8">
      <div className="max-w-4xl mx-auto bg-white shadow-2xl rounded-xl overflow-hidden">
        <div className="bg-slate-900 text-white p-8 md:p-12">
          <h1 className="text-4xl font-bold mb-2">{data.name || 'Name Not Provided'}</h1>
          <p className="text-xl text-slate-300 font-medium mb-6">{data.title || 'Professional Title'}</p>
          <div className="flex flex-wrap gap-4 text-sm text-slate-400">
            {data.phone && <span>{data.phone}</span>}
            {data.linkedin && (
              <a href={formatUrl(data.linkedin)} className="hover:text-white underline">
                {data.linkedin}
              </a>
            )}
